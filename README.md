...
离线时可继续匹配已拉取的数据；遇到需要拉取的章节、想法批次或原文时暂停并提示联网，保留进度。
选章界面按书本目录展示层级，点箭头展开或折叠，勾选父项会同时选中子项，取消也会联动。
子项可以单独选择，父项以短横线表示部分选中；底部固定显示匹配按钮与已选章节数。
已匹配章节保留在目录中，但不可勾选；父项全选会跳过它们，计数只包含待匹配章节。
