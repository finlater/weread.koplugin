...
并发下载使用 KOReader 的隔离子进程执行阻塞网络请求，并在主界面异步回收结果。自动模式会读取 `/proc/meminfo`，评论和图片按设备总内存选择 1–6 路预设；章节接口依赖连续的阅读会话，自动模式固定串行以防内容截断。手动模式仍可分别设置 1–8 路并发，章节并发结果会校验正文长度，异常时自动串行重试；检测失败或平台不支持子进程时全部退回串行。
