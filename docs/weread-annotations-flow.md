...
选章使用当前文档的目录层级，保留没有远端对应章节但包含可选子章的父标题。
没有本地目录映射时回退到微信目录的 level。父项选择包含自身对应章节和全部
可选子章，子项可独立选择，父项显示部分选中状态。选择状态只在弹窗内存中存在；
按页创建和释放控件，不为整本目录创建隐藏控件。
已匹配章节只读展示，父项联动和已选数量仅包含未匹配章节；底部操作栏使用
KOReader 原生 ButtonTable 的等宽按钮、分隔线和留白。
