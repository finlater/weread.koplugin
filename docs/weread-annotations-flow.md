...
微信读书 gateway API
  ...
  原始章节 HTML ──► <a href="#wrthought-BOOK-UID-RANGE"><span wr-underline>划线</span></a>
        │            已确认有想法的 range 才会带 <span class="wr-star">*</span>
...
- `wrapTextSegments`：区间内的**文本段**逐段用 `<span class="wr-underline">` 包裹，遇标签自动断开重开。
- **每条划线**都包上内部链接 `<a class="wr-thought-link" href="#wrthought-BOOK-UID-RANGE">`（不使用 `epub:type="noteref"`，避免进入 KOReader 内建脚注路径），以便按需点击补拉。
- **星号**只加在下载时已经确认有想法正文的 range 上。按需下载的书通常没有 `*`，下划线本身可点。
...
- `.wr-star`：灰色小星号上标。
- `.wr-thought-link`：可点击，无额外下划线装饰。

处理后的 HTML + 注释 CSS 经 `Thoughts.merge_css` 合并，由 `Content.save_book_epub` 打包。
...
| `weread/lib/annotations.lua` | 注入下划线链接；仅给已确认想法的 range 标星 |
