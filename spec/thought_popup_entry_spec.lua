...
test("reopening the default position reuses the centered pool", function()
    ...
    eq(reopen_log[1].position, nil, "position not forwarded when unset")
    eq(reopen_log[1].width_ratio, nil, "width ratio not forwarded when unset")
end)

test("explicit bottom position constructs the bottom widget and drops the centered pool", function()
    local popup = M.show({ pages = pages1, position = "bottom" })
    eq(#created_widgets, 1, "one bottom widget created")
    eq(popup, created_widgets[1], "returned widget is the bottom one")
    eq(popup.items, pages1, "bottom widget receives pages as items")
    eq(#freed_content_log, 1, "centered pool content caches freed on switch")
    eq(freed_content_log[1], created_centers[1], "centered widget was freed")
end)

test("reopening the bottom position reuses its pool", function()
    local popup = M.show({ pages = pages2, position = "bottom" })
    eq(#created_widgets, 1, "no second bottom widget created")
    eq(popup, created_widgets[1], "same bottom widget reused")
    eq(popup.items, pages2, "reopened bottom content updated")
    eq(reopen_log[#reopen_log].position, "bottom", "position forwarded")
end)

test("switching back to the default position frees the bottom pool", function()
    M.show({ pages = pages1 })
    eq(#created_centers, 2, "centered pool rebuilt after switch")
    eq(#freed_content_log, 2, "bottom pool content caches freed")
    eq(freed_content_log[2], created_widgets[1], "bottom widget was freed")
end)
